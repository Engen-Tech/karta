...
The queue is executed by `skills/karta-deliver/scripts/merge_item.py` — one `merge` invocation per item performs these steps mechanically, stops at the first failure, and prints one fixed-shape JSON result; `close-wave` runs the post-wave checks (each an explicit `--check` command, then the shared-terms check) and never writes a tag; `tag-wave` writes the wave success tag after the wave's accept/defer decisions resolve. The script changes how the queue is run, not what the queue does.
...
**The accept merge is made with `--no-ff` and carries the item marker in its subject:** `Accept item <id> into integration [karta:item-<id>]`, with the two `Karta-Accepted` + `Karta-Accept-Reason` trailers below it — the same marker grammar as the queue merge. A fast-forward leaves no merge commit to carry the subject and trailers, and the narrow `<done>^1..<done>` provenance range that resume and the queue's done-ref check depend on is then meaningless; an accept merge whose subject lacks the marker fails that range with "no marker". An accept merge made before this rule is unmarked, so a resumed delivery containing one halts at done-provenance — the way forward is Clear (the binder is re-delivered), never a re-stamping of history.
