...
2. **Check provenance before the merge.** Run `uv run --script <skill-dir>/scripts/check_item_provenance.py --repo <worktree> --item <id> --range <integration-tip>..<item-tip>` — **without** `--check-accepted`, because the `accepted` ref and the `done` merge are written only *after* a merge lands, so pre-merge is the wrong lifecycle point for that flag. A nonzero result halts that item's merge with the checker's findings; provenance is a command here, not a prose expectation.
3. **Re-validate the oracle against the current integration tip** (which may have advanced as wave-mates merged), through the same deterministic runner the worker used: `uv run --script <skill-dir>/../karta-build/scripts/run_oracle.py --cwd <resolved cwd> [--expect <oracle.expect>] '<oracle command>'`, then read the emitted record's `success`, `exit_status`, and `decisive_output`. **The orchestrator never reads full oracle logs.** It re-checks here — it does **not** trust the worker's verdict; the `built` marker only says the worker finished a clean run on its own branch, not that the item still passes on the moved tip.
4. **Merge re-validation always re-executes.** The runner re-runs the oracle against the merged tip every time. There is no skip-on-match: a matching command hash says nothing about the composed tree, which is exactly what merge-time re-validation exists to test. The stored evidence record (`git cat-file -p refs/karta/<slug>/item-<id>/evidence`) serves two purposes here and no others — its `command_sha256` is compared against the hash of the binder oracle's current command as a **drift check** (a mismatch means the binder's command changed since the item built: surface it, never silently accept it), and a missing or unreadable evidence ref is **noted, then ignored** — a read failure never blocks the re-run, which proceeds by execution as ever.
5. Merge (ff or no-ff). On a merge conflict or a re-validation failure, do a bounded rebuild against the new tip or halt.
...
- **Shared-terms drift halts the wave too.** The post-wave step runs `check_shared_terms.py --binder <binder path>` on the integration tip after the build/type-check; a `[FAIL]` (a declared canonical string drifted between landed items) reverts the wave on the same footing as a failed build, while `[PENDING]` entries for items in later waves are skipped, not failed.
