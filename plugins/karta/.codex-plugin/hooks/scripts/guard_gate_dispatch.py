...
# dependencies = []
# ///
"""Codex PreToolUse guard: gate-reviewer dispatches carry a real, sized diff.

Hand-maintained Codex twin of hooks/scripts/guard_gate_dispatch.py (the
...
use underscores: both spellings of either gate are accepted
(`karta-acceptance-reviewer` / `karta_acceptance_reviewer`,
`karta-safety-auditor` / `karta_safety_auditor`) in any identity field.
Recognition is a substring test on those FIELDS only; a prose mention of a
gate inside message or prompt text is NOT recognition. Stated limit: a
fallback dispatch carrying a generic agent_type and no karta-bearing
task_name is not recognized — the enforced claim is scoped to dispatches that
identify their gate in a payload identity field.

For a recognized dispatch it requires, in the joined text: a parseable diff
range (a `<rev>..<rev>` or `<rev>...<rev>` token — karta-verify already
mandates the range as a required input, so its absence is a malformed brief,
not a novel style), a diff range that is not empty (`git diff --quiet
<range>` reports changes), and a `Diff-size: <files> files, <bytes> bytes`
line whose numbers match what git recomputes for that range. Missing range,
unresolvable range, empty range, missing Diff-size line, or a mismatched
Diff-size line all deny — the empty-diff refusal moves the existing
empty-diff-is-BLOCKED doctrine from a post-hoc gate verdict to a pre-dispatch
refusal, saving two reviewer contexts per occurrence, and the Diff-size line
makes an oversized diff visible before anyone reads it. A malformed
`worktree` mention denies rather than falling back to the payload cwd.
...
GATE_NAMES = ("karta-acceptance-reviewer", "karta_acceptance_reviewer",
              "karta-safety-auditor", "karta_safety_auditor")
...
def _recognized(tool_input: dict) -> bool:
    for k in IDENTITY_KEYS:
        v = tool_input.get(k)
        if isinstance(v, str) and any(agent in v for agent in GATE_NAMES):
            return True
    return False
...
def _run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # first grammar in the Claude original stopped the rev class at every dot, so
        ...
        # on the sibling and the range fails to resolve.
        ...
        def dispatch(prompt: str, subagent: str = "karta-safety-auditor",
                     worktree: str = repo, key: str = "subagent_type",
                     text_key: str = "prompt") -> dict:
            return {"hook_event_name": "PreToolUse", "tool_name": "spawn_agent",
                    "cwd": worktree, "tool_input": {key: subagent, text_key: prompt}}
        ...
        flag("_CLAUSE_PUNCT is a subset of _WRAP_PUNCT and no path-like marker is in "
             "_WRAP_PUNCT", set_ok)

        # The Claude original's fixture set, carried over.
        check("unrecognized subagent passes with no diff-size line",
              dispatch("build item a", subagent="karta-build"), 0, None)
        check("mention of the gate agents in prose alone is not recognition",
              dispatch("after the build, karta-safety-auditor scans it",
                       subagent="karta-build"), 0, None)
        check("recognized acceptance-reviewer, well-formed range and Diff-size line passes",
              dispatch(f"worktree {repo}; diff range base..feature. {good_size_line}",
                       subagent="karta-acceptance-reviewer"), 0, None)
        check("recognized safety-auditor, well-formed range and Diff-size line passes",
              dispatch(f"worktree {repo}; diff range base..feature. {good_size_line}"), 0, None)
        check("a dotted ref resolves — the rev class allows dots without eating the separator",
              dispatch(f"worktree {repo}; diff range base..release/2.31.0. {good_size_line}"),
              0, None)
        check("a trailing full stop is not part of the endpoint",
              dispatch(f"worktree {repo}; diff range base..feature. {good_size_line}"), 0, None)
        check("a worktree path ending in a dot is never guessed at — it denies rather "
              "than resolving a different tree",
              dispatch(f"worktree {repo}. diff range base..feature. {good_size_line}",
                       worktree=missing_dir), 2, "could not resolve")
        check("a traversal path keeps its dots — they are the path, not punctuation",
        ...
        check("negative control: the sibling really lacks those branches, so the case "
              "above passed for the right reason",
              dispatch(f"worktree {twin_plain} diff range base..feature. {good_size_line}",
                       worktree=missing_dir), 2, "could not resolve")
        check("clause punctuation AFTER such a directory is peeled one character at a time, "
              "stopping at the directory rather than overshooting to its sibling",
              dispatch(f"worktree {twin_punct}; diff range base..feature. {good_size_line}",
                       worktree=missing_dir), 0, None)
        check("a quoted directory of that kind followed by clause punctuation is unquoted "
              "at the right step",
              dispatch(f'worktree "{twin_punct}"; diff range base..feature. {good_size_line}',
                       worktree=missing_dir), 0, None)
        check("a quoted form of it with extra clause punctuation inside the quotes is "
              "unquoted first, then peeled to the directory",
              dispatch(f'worktree "{twin_punct};" diff range base..feature. {good_size_line}',
                       worktree=missing_dir), 0, None)
        check("an UNMATCHED leading quote is a malformed mention: denied outright, even "
              "though the payload cwd is a real tree that resolves the range",
              dispatch(f'worktree "{twin_punct} diff range base..feature. {good_size_line}',
                       worktree=repo), 2, "malformed")
        check("a correctly quoted path containing a space is cut at the space and denied "
              "outright, not judged against the payload cwd",
              dispatch(f'worktree "{td}/my tree" diff range base..feature. {good_size_line}',
                       worktree=repo), 2, "malformed")
        check("a path behind a bracket AND a quote is still a malformed mention, not prose",
              dispatch(f'worktree ("{twin_punct}; diff range base..feature. {good_size_line}',
                       worktree=repo), 2, "malformed")
        check("a bare relative path with no leading ./ names something and cannot be read: "
              "denied, not judged against the payload cwd",
              dispatch(f"worktree foo/bar diff range base..feature. {good_size_line}",
                       worktree=repo), 2, "malformed")
        check("a dot form behind brackets names the parent, and cannot be read: denied, "
              "not judged against the payload cwd",
              dispatch(f"worktree (..) diff range base..feature. {good_size_line}",
                       worktree=repo), 2, "malformed")
        check("a bare word after an explicit separator is a name, not prose: denied",
              dispatch(f"worktree: release diff range base..feature. {good_size_line}",
                       worktree=repo), 2, "malformed")
        check("a quoted bare word is a name, not prose: denied",
              dispatch(f'worktree "release" diff range base..feature. {good_size_line}',
                       worktree=repo), 2, "malformed")
        check("a bare word with no separator, quote, bracket or slash is prose and the "
              "payload cwd stands in — the one reading the guard still makes",
              dispatch(f"worktree release; diff range base..feature. {good_size_line}",
                       worktree=repo), 0, None)
        check("a trailing `worktree:` with nothing after it is a separator naming nothing "
              ...
              dispatch(f"diff range base..feature. {good_size_line}\nworktree:",
                       worktree=repo), 2, "malformed")
        check("same with `=` and surrounding spaces",
              dispatch(f"diff range base..feature. {good_size_line} worktree = ",
                       worktree=repo), 2, "malformed")
        check("a prose word holding a slash is denied too — fail-closed, one re-dispatch",
              dispatch(f"worktree and/or branch diff range base..feature. {good_size_line}",
                       worktree=repo), 2, "malformed")
        check("a non-string payload cwd is a malformed payload and denies, not a TypeError "
              "into the outer fail-open",
              {"hook_event_name": "PreToolUse", "tool_name": "spawn_agent", "cwd": {"a": 1},
               "tool_input": {"subagent_type": "karta-safety-auditor",
                              "prompt": f"worktree {repo}; diff range base..feature. "
                                        f"{good_size_line}"}}, 2, "not a string")
        check("a NUL in the path is a fail-closed deny, not an exception that fails open",
              dispatch(f"worktree {repo}\x00x; diff range base..feature. {good_size_line}",
                       worktree=missing_dir), 2, "fails closed")
        check("a relative worktree form is resolved against the payload cwd",
              dispatch(f"worktree ./ diff range base..feature. {good_size_line}",
                       worktree=repo), 0, None)
        check("a file at the longer name stops the peel and denies rather than peeling "
        ...
        check("the word after a prose 'worktree' is not taken as a path",
        ...
        check("empty range denies, naming the whiff/already-present causes",
              dispatch(f"worktree {repo}; diff range base..base. {good_size_line}"),
              2, "whiff")
        check("missing range denies",
              dispatch(f"worktree {repo}; {good_size_line}"), 2, "diff range")
        check("missing Diff-size line denies, printing the recomputed stat",
              dispatch(f"worktree {repo}; diff range base..feature."),
              2, f"Diff-size: {real_files} files, {real_bytes} bytes")
        check("mismatched Diff-size numbers deny, distinct from the missing-line case",
              dispatch(f"worktree {repo}; diff range base..feature. {bad_size_line}"),
              2, "fictional stat")
        check("the mismatch deny carries the verbatim recomputes fragment",
              dispatch(f"worktree {repo}; diff range base..feature. {bad_size_line}"),
              2, f"recomputes Diff-size: {real_files} files, {real_bytes} bytes")
        check("a mismatched three-dot range denies and is split on its own separator",
              dispatch(f"worktree {repo}; diff range base...feature. {bad_size_line}"),
              2, "recomputes Diff-size:")
        check("unresolvable worktree/range is an internal-error fail-closed deny",
        ...
        check("tool_input not a dict passes",
              {"hook_event_name": "PreToolUse", "tool_name": "spawn_agent", "cwd": repo,
        ...
        check("underscore acceptance-reviewer spelling in agent_type is recognized",
              dispatch("review the item", subagent="karta_acceptance_reviewer",
                       key="agent_type", text_key="message"), 2, "diff range")
        check("underscore safety-auditor spelling in agent_type is recognized",
              dispatch("audit the boundaries", subagent="karta_safety_auditor",
                       key="agent_type", text_key="message"), 2, "diff range")
        check("underscore task label alone is recognition (fallback-agent dispatch)",
              dispatch("review the item", subagent="karta_acceptance_reviewer",
                       key="task_name", text_key="message"), 2, "diff range")
        check("range and Diff-size carried solely in task_name are joined into the "
              "inspected text",
              {"hook_event_name": "PreToolUse", "tool_name": "spawn_agent", "cwd": repo,
               "tool_input": {"agent_type": "karta-acceptance-reviewer",
                              "task_name": f"review base..feature {good_size_line}"}},
              0, None)
        check("prose mention of a gate inside message text is not recognition",
              dispatch("please have the karta-acceptance-reviewer look at base..feature",
