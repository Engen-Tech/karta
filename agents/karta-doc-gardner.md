...
focus_stale: "one-line advisory"       # optional — only when the note HAS concrete tokens and none of them name anything in this delivery's blast radius; omitted otherwise
```
...
- **Focus is a lens, never a fence — staleness is observed, never fixed.** The focus note biases your attention and never bounds your sweep; drift outside it is corrected exactly like drift inside it. When the note has concrete tokens and none of them name anything in the blast radius, return the one-line `focus_stale` advisory in your envelope (a note with no concrete tokens never flags) — and never edit `.karta/` to fix the note, because the consumer's config is not your writer surface.
