...
# dependencies = []
# ///
"""Codex SessionStart hook: inject a short karta status summary as session context.

This is the hand-maintained Codex twin of `hooks/scripts/inject_karta_status.py`
— a port, never byte-compared with the Claude original. Zero dependencies (pure
stdlib). Codex invokes this on its session-start event with the hook payload
JSON on stdin. Codex documents two alternative encodings of the same stdout
stream for injected context — plain text, and the
`hookSpecificOutput.additionalContext` JSON field; this twin commits to the
PLAIN-STDOUT form only: the fenced block as bare text, never a JSON wrapper and
never both (concatenation would corrupt a JSON-parsing host and double-inject
on a text-reading one). When `<cwd>/.karta/binders/*.json` exists it emits at
most one short line per binder (slug, item count, pinned packs), preferring the
karta-status derivation script (`skills/karta-status/scripts/karta_next.py
--json`) for live status and the next action, resolved via CLAUDE_PLUGIN_ROOT
(which Codex sets for compatibility) with a fallback that finds this script's
own plugin root from `__file__` by EXISTENCE TEST — walking parent directories
until one actually contains the `skills/` tree, never a fixed parent depth: the
Claude original's directory sits one level shallower, so a copied depth would
point at `.codex-plugin/` instead of the plugin root. It degrades to a static
binder summary when the derivation script is not invocable, exits non-zero, or
emits unusable output — the static path is a sanctioned outcome, not an error.
Output stays within 10 lines total; silence when there are no binders. Always
exits 0 — a status hint must never surface as a session error, and Codex caps
injected context at additionalContextLimit (default 2500 tokens), which this
script's own budgets keep typical output well under.
...
# Keep in step with the Claude original's BYTE_BUDGET (which is itself pinned
# to injection_byte_budget in benchmarks/fixtures/adversarial/expected.json) —
# the two twins share one injection budget by hand-maintained agreement.
BYTE_BUDGET = 4096
...
def _own_plugin_root() -> Path | None:
    """Find this script's own plugin root by EXISTENCE TEST: walk parent
    directories of `__file__` until one actually contains the `skills/` tree
    (probed as `skills/karta-status/scripts`, the subtree every consumer of
    this root resolves into). Never a fixed parent depth: the Claude original
    sits at `<root>/hooks/scripts/` and its copied depth would land on
    `.codex-plugin/` here (this twin sits at `<root>/.codex-plugin/hooks/
    scripts/`) — and the marketplace projection nests one level deeper still.
    None when no parent qualifies (the caller degrades)."""
    d = Path(__file__).resolve().parent
    while True:
        if (d / "skills" / "karta-status" / "scripts").is_dir():
            return d
        if d.parent == d:
            return None
        d = d.parent
...
def derive_state(cwd: str) -> dict | None:
    ...
    try:
        proc = subprocess.run([sys.executable, str(script), "--json"],
                              capture_output=True, text=True, cwd=cwd, timeout=15, encoding="utf-8", errors="replace")
...
def _watch_self_test_checks() -> list[tuple[str, bool]]:
    ...
    # Pin the plugin root at this checkout so both this process and the hook
    # subprocesses resolve the watch scripts above this file — never an
    # installed plugin cache a live session may point CLAUDE_PLUGIN_ROOT at.
    # Resolved by the existence-test walk, NOT a fixed parent depth: this twin
    # sits one level deeper than the Claude original.
    saved_root = os.environ.get("CLAUDE_PLUGIN_ROOT")
    _pinned_root = _own_plugin_root()
    if _pinned_root is not None:
        os.environ["CLAUDE_PLUGIN_ROOT"] = str(_pinned_root)
...
def _run_self_test() -> int:
    ...
    # -- Codex-twin specifics: plugin-root walk and hook-mode error paths --
    root = _own_plugin_root()
    checks.append(("plugin root resolves by existence test (skills/ tree found)",
                   root is not None
                   and (root / STATUS_REL).is_file()
                   and (root / WATCH_REL).is_file()))
    # Companion negative: the Claude original's fixed parent depth, copied
    # here, would land on `.codex-plugin/` (or the projection's), which does
    # NOT contain the skills tree — the walk is what makes resolution work.
    fixed_depth = Path(__file__).resolve().parent.parent.parent
    ...
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as td:
        binders_dir = Path(td) / ".karta" / "binders"
        binders_dir.mkdir(parents=True)
        (binders_dir / "good.json").write_text(json.dumps(_binder_fixture("good-slug", [])), encoding="utf-8")
        (binders_dir / "zbroken.json").write_text("{ not json", encoding="utf-8")
        me = str(Path(__file__).resolve())
        proc = subprocess.run([sys.executable, me],
                              input=json.dumps({"hook_event_name": "SessionStart",
                                                "cwd": td}),
                              capture_output=True, text=True, timeout=120, encoding="utf-8")
        checks.append(("hook e2e: malformed binder beside a good one exits 0,"
                       " good one still emitted in one intact fence",
                       proc.returncode == 0 and "good-slug" in proc.stdout
                       and proc.stdout.count(_DELIM_CLOSE) == 1))
        checks.append(("hook e2e: non-git cwd exits 0 and still emits",
                       not (Path(td) / ".git").exists()
                       and proc.returncode == 0 and _DELIM_OPEN in proc.stdout))
        checks.append(("hook e2e: plain-stdout form only — no JSON wrapper",
                       "hookSpecificOutput" not in proc.stdout
