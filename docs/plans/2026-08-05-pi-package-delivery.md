...
|Package|`@engen-tech/karta` version `2.35.0`, private and `UNLICENSED`|
|Completed|Phase 0 feasibility closure and Phases 1–6 for the 2.35.0 macOS/Linux support scope|
|In progress|Release approval only; native Windows support is deferred beyond 2.35.0|
...
|Commit state|The Phase 6 candidate and narrowed 2.35.0 support scope are committed; packed-artifact local acceptance is the final pre-remote gate on `feat/pi-package`|
...
- current macOS;
- Linux;
- native Windows is deferred beyond 2.35.0 and is not a release gate for this version;
