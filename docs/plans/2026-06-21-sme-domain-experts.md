...
**Goal:** Give karta a curated, auto-applied domain-expertise layer. stack packs come in two kinds — **stack packs** (matched by tech: angular, python-fastapi) and **always-on stack packs** (apply to every binder: minimalism, a minimalism ladder). Each pack's diff-checkable Review checklist is enforced by the existing safety-auditor. v2 also adds debt mechanics (a ceiling/upgrade marker grammar, a no-trigger rot flag, a read-only `karta-debt` harvest skill) and a `benchmarks/sme/` method to prove a pack helps.

**Architecture:** stack packs are markdown reference files (built-in under `skills/_shared/sme/`, project overlay under `.karta/sme/`). A pack is a **stack pack** (`match: [tokens]`) or a **always-on stack pack** (`always: true`). karta-plan applies always-on stack packs to every binder and stack packs by match, then pins all applied ids in the binder's `sme[]` field. karta-build loads the applied packs to write against and self-checks its diff before commit, recording declared overrides as inline `KARTA-SME-OVERRIDE(<pack>: <rule>): <reason> [ceiling: …; upgrade: …]` markers. karta-verify resolves the pinned packs' Review checklists and passes them to `karta-safety-auditor`, which — only when `sme[]` is non-empty — flags any **undeclared** checklist violation as a boundary VIOLATION. The acceptance-reviewer stays stack-pack-unaware; one acceptance authority. A `platform-native` reference of native/stdlib substitutions ships as a shared reference (`skills/_shared/sme/platform-native.md`) that the minimalism + stack packs link to (reference, don't copy).
...
- **One acceptance authority.** `karta-acceptance-reviewer` stays stack-pack-unaware. stack-pack enforcement is one **conditional** check on `karta-safety-auditor`, active only when the binder's `sme[]` is non-empty. No new gate; no new *agent* (the new `karta-debt` skill is read-only and dispatches nothing).
- **Two pack kinds, one mechanism.** Stack packs carry `match: [tokens]`; always-on stack packs carry `always: true`. They differ only in *selection* — both pin in `sme[]`, both write against the implementer, both are enforced through the Review checklist. The first always-on stack pack is `minimalism`. **No intensity dial.**
- **Checklist items must be diff-checkable.** Every `## Review checklist` item (stack or rule) must be objectively checkable on a diff. Heuristics ("prefer the simplest thing", the ladder) stay in Do / Don't / Patterns / advisory sections. This is what lets the minimalism always-on stack pack be enforced without the gate judging a vibe.
...
- `skills/_shared/sme/minimalism.md` — **always-on stack pack** (`always: true`): minimalism ladder (advisory) + safety floor + diff-checkable Review checklist.
...
- `skills/karta-plan/SKILL.md` — config row, matcher (always-on stack packs always-apply + stack packs by match), domain-guidance, pin, report line.
...
- `agents/karta-safety-auditor.md` — conditional stack-pack conformance check.
...
- `AGENTS.md`, `README.md` — layout rows + stack-packs section (both pack kinds, minimalism, karta-debt, measurement).
...
- [ ] **Step 2b: Write the `minimalism` always-on stack pack** (`skills/_shared/sme/minimalism.md`)
...
Run:
```bash
# stack + always-on stack packs carry a name and a Review checklist
...
# selection: stack packs match; the always-on stack pack is always:true
...
```json
    ...
    "sme": {
      ...
      "description": "ids of advisory stack packs auto-matched at plan time; the Review checklist of each is the enforceable subset the safety-auditor judges (project-local .karta/sme/<id>.md overrides the built-in references/sme/<id>.md)"
    },
    "token_manifest": {
```
...
```
| `design_facts.stack` | string | no | Resolved tech stack, recorded once at plan time |
| `sme` | string[] | no | Ids of advisory stack packs matched at plan time; the implementer writes against them and `karta-safety-auditor` enforces each pack's Review checklist (undeclared violations only). Absent or `[]` = none |
```
...
- Produces: the binder's `sme[]` (matched ids), and stack-pack do's/don'ts threaded into the synthesis brief.

- [ ] **Step 1: Add the stack-pack config row**
...
2. Select the packs that apply, by frontmatter kind:
   - A **always-on stack pack** (`always: true`, e.g. `minimalism`) applies to **every** binder, unconditionally.
...
grep -q "stack pack matching" skills/karta-plan/SKILL.md && echo matcher-ok
...
- [ ] **Step 2: Add the stack-pack config row to build's Project configuration table**
...
**9-sme. stack-pack self-check before commit (when `SME_PACKS` is non-empty).** For each loaded pack (4c-ter), run its **Review checklist** against the item's diff (`git diff "$integration"...HEAD`). For every checklist item, decide pass or miss. Two outcomes for a miss:
...
- [ ] **Step 5: Add the stack-pack report bullet to `build:report`**
...
```
- **Acceptance result** — which gate ran (`karta-verify` / `karta-validate` / opted out), final disposition, rounds used, any residual finding
- **stack-pack self-check** — per applied pack, the Review-checklist tally and any `KARTA-SME-OVERRIDE` declared (what, which rule, why); plus a note for any pinned `sme` id that resolved to no pack file. Omit the whole line when `SME_PACKS` is empty
```
...
- [ ] **Step 3: Add the conditional stack-pack check after the seven signals**
...
```bash
grep -q "stack-pack conformance" skills/karta-verify/references/karta-safety-auditor.agent.md && echo bundle-ok
grep -q "stack-pack conformance" .codex/agents/karta-safety-auditor.toml && echo toml-ok
```
...
- `KARTA-SME-OVERRIDE(<pack>: <rule>): <rationale> [ceiling: <limit>; upgrade: <trigger>]` — a deliberate deviation from a stack pack's Review checklist (see karta-build, `build:merge`).
...
```python
#!/usr/bin/env python3
"""Measure a stack pack's effect: compare an A (pack applied) vs B (pack absent) diff.
...
def main() -> int:
    ...
    print(f"## stack pack A/B — {a.label}")
...
```markdown
# stack pack benchmarks

Measure whether a stack pack earns its place: run the same fixture task **with the pack** and **without it**, then compare.
...
- Modify: `README.md` (new stack-packs section)
...
- [ ] **Step 2: Add a short stack-packs section to the README**
...
with:
```
## Domain experts (stack packs)

karta carries curated **stack packs** so planning and implementation follow good norms. There are two kinds:
...
git commit -m "docs(sme): document stack packs in README and AGENTS layout"
```
...
**Spec coverage (v2):**
- Pack format + built-ins, incl. the `minimalism` always-on stack pack and the shared `platform-native` reference (spec 3a, 3b, 3g) → Task 1.
...
- Plan match (always-on stack packs always-apply + stack packs by match) + guidance + pin + report (spec 3c, 3d) → Task 4.
...
- Non-goals (spec 5): no new gate authority (auditor conditional, acceptance-reviewer untouched ✓), checklist items diff-checkable so the always-on stack pack is enforceable without judging a vibe ✓, no intensity dial ✓, no tracked backlog (rot flag + karta-debt are read-only/one-shot ✓), no per-item override ✓.

**Placeholder scan:** every code step shows full content; every command shows expected output. No TBD/TODO.

**Type/name consistency:** field `sme` (array of kebab-case strings, holds stack + always-on stack pack ids); pack ids `angular`, `python-fastapi`, `minimalism`; selection `match` (stack) vs `always: true` (rule); shared reference `platform-native.md`; cache var `SME_PACKS`; phase label `plan:sme`; build steps `4c-ter` / `9-sme`; marker `KARTA-SME-OVERRIDE(<pack>: <rule>): <reason> [ceiling: …; upgrade: …]` — used identically across Tasks 3-9. The auditor receives checklists as a conditional 4th input; karta-verify is the sole resolver/dispatcher; `karta-debt` dispatches nothing.
