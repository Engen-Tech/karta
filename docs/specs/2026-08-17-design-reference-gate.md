...
Rung 4 is opt-in inside an opt-in: `recapture_after` is optional, so no existing pin gains a deadline it did not ask for, and a capture that outlives the life its author gave it stops the comparison instead of being disclosed and ignored. The rung keys on the value being non-null rather than on it being truthy: `null` is how JSON writes "absent", so a generator that always emits the key still means no deadline, but a present-but-falsy `recapture_after` (`0`, `""`, `false`) is a deadline written down wrong, and reading that as "no deadline" would let a pin outlive the life its author tried to give it — the one thing the rung exists to stop.
