...
| Pack edits must validate | Claude Code (plugin hook) | A `Write` of a `.md` file under `.karta/sme/` is checked against the pack validator before it lands and blocked with the findings. After an `Edit` or `Write`, the file on disk is validated again; a failure comes back as feedback the agent must fix. |
...
| Commits in this repo pass the gate suite | Claude Code (this repo's project settings) | A `git commit` in a karta checkout first runs the repo's sync and validation gates, then validates every live binder the commit would record; a failure blocks the commit with its output. Not shipped in the plugin. |
...
The first eight ship in the plugin: install karta and they are active in every project you use it in. The last two live in this repository and protect karta's own development. Seven of the plugin rules also ship Codex-side twins — see the runtime parity table in [the Codex how-to](codex.md).
...
If a script hits an internal error, it allows the call: enforcement must never break normal work. The three exceptions are the guards whose whole point is to fail closed — `guard_auditor_dispatch.py`, which blocks a dispatch it recognizes unless the required evidence is present; `guard_gate_dispatch.py`, which blocks a gate-reviewer dispatch it recognizes unless the diff range, its non-emptiness, and its Diff-size line all check out; and `guard_writer_confinement.py`, which blocks a write from a confined writer (kaizen or doc-gardner) it recognizes unless the target is inside that writer's surface. Any shape they do not recognize passes. Every script has a `--self-test`, and `validate_plugin.py` checks the manifest, the scripts, and their self-tests.
...
The commit gate exists because every pack and skill in this repo has generated mirror copies, and nothing else checks them at commit time. On `git commit` it runs six gates — `check_shared_copies.py`, `check_invariant_register.py` (the carrier files and quoted phrases `docs/conventions/invariants.md` names), `sync_codex_skills.py --check`, `sync_codex_agents.py --check`, `validate_plugin.py`, and the pack validator over `skills/_shared/sme/` and `.karta/sme/` — then validates every live binder the commit would record, against the exact bytes git records for that commit shape. The first failure blocks the commit with its output. Every one of those reads the tree the commit is issued in, so a commit made in a linked worktree is judged on that worktree.
