...
- **Linked worktrees are first-class, since 2026-08-30.** A commit issued from the **top level of a linked worktree** of this repository is allowed, and the gate rescopes every git and file lookup to that worktree, so the commit is judged on the tree git will actually commit — a worktree commit that stages a binder still needs its fresh record, exactly as at the root. A subdirectory of a worktree is denied like a subdirectory of the root, and a directory `git worktree list` does not name is denied outright. This exists because the flat everything-but-ROOT denial the gate first shipped with was a regression: karta's own wave workers commit item branches from worktrees, and the first delivery after that gate landed (`context-economy`, wave 1) halted two of three builds at their final `git commit` — every review round of the ledger-gate change missed it because no oracle exercised a commit from a worktree. The fix rescopes rather than exempts, so the gate lost no force.
