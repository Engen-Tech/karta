...
Status describes the strongest install — Claude Code with the plugin's hooks. On a Codex plugin
install several **enforced** rows degrade to doctrine backed by the OS sandbox; README's
"Enforcement below the agent" states that split once so this file doesn't overclaim it 27 times.
...
- founding · **enforced** — write hooks on Claude Code; `sandbox_mode = "read-only"` derived for Codex gate agents; README "Enforcement below the agent" names the Codex degradation.
