...
2. **Check provenance before the merge.** Run `python3 skills/karta-deliver/scripts/check_item_provenance.py --repo <worktree> --item <id> --range <integration-tip>..<item-tip>` — **without** `--check-accepted`, because the `accepted` ref and the `done` merge are written only *after* a merge lands, so pre-merge is the wrong lifecycle point for that flag. A nonzero result halts that item's merge with the checker's findings; provenance is a command here, not a prose expectation.
3. **Re-validate the oracle against the current integration tip** (which may have advanced as wave-mates merged), through the same deterministic runner the worker used: `python3 skills/karta-build/scripts/run_oracle.py --cwd <resolved cwd> [--expect <oracle.expect>] '<oracle command>'`, then read the emitted record's `success`, `exit_status`, and `decisive_output`. **The orchestrator never reads full oracle logs.** It re-checks here — it does **not** trust the worker's verdict; the `built` marker only says the worker finished a clean run on its own branch, not that the item still passes on the moved tip.
