...
Every change lands as a labeled `kaizen:` commit on the branch you already review — no PR, no push. Kaizen writes knowledge and never changes what gates a build. Sharpening a rule from repeated overrides runs today; suggesting a new pack is deferred to a later phase. It ships the **`karta-kaizen`** skill and a writer agent confined to your packs. Full guide: [`docs/how-to/kaizen.md`](docs/how-to/kaizen.md).
