...
import { buildKartaEvidence, type KartaEvidenceManifest } from "./evidence.ts";
import {
...
export type EvidenceBuilder = (options: {
  cwd: string;
  binder: string;
  item: string;
...
export class KartaVerificationRunner {
  ...
  async run(
    ...
    try {
      const evidence = await this.#buildEvidence({ cwd: ctx.cwd, binder, item });
