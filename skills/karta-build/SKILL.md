...
**With an `item_context.py` packet in the dispatch brief (a wave dispatch — see karta-deliver's Step 2), Gates 1-3 below and the Extract-and-cache step are satisfied FROM IT.** The orchestrator's own preflight (`deliver_preflight.py`) already validated the binder, so Gate 1 is met; the packet's `item` field, present and non-null, is Gate 2; its `dependencies` map — `{dep_id: {done_ref, target}}` — answers Gate 3 directly (a null `target` is an unmet dependency). Read `oracle_cwd`, `oracle_expect`, `dependencies`, and `sme` (with its already-composed checklists) straight off the packet instead of re-deriving them: do not re-run `validate_binder.py`, and do not search the filesystem for its path or any other script's. **Without a packet — a directly-invoked worker — run the three gates below as written; nothing about that path changes.**

Three gates, **all must pass**. Any failing is an immediate hard stop — report and exit, no "continue anyway?".

**Gate 1 — The binder validates.** Run `uv run --script <skill-dir>/../karta-plan/scripts/validate_binder.py --binder <binder path>` (schema validity, dependency cycles, dangling `depends_on`). If the orchestrator already validated the binder for this run, take that as satisfied rather than re-running — but never skip validation when invoked directly. On failure, bail with the validator's output.

**Gate 2 — The item id exists.** Find the work item whose `id` equals the requested id in the binder's `work_items`. If none matches, bail with the requested id and the list of available ids.

**Gate 3 — Dependencies are merged.** Every id in `depends_on` must already be merged into the integration branch — i.e. its done ref exists (`refs/karta/<slug>/item-<dep-id>/done`; see [references/integration-branch.md](references/integration-branch.md)). Unmet deps **halt with a call to action** naming them; `depends_on` is a scheduling constraint, not something to build past.

**Resolve git identity** (Project configuration) for commit authorship only — no ticketing system, assignee, or status field. If `git config` has no `user.email`/`user.name`, ask once or record an explicit "unattributed" note.

**Extract and cache** from the item (or the packet's `item`/`dependencies`/`sme` fields, when present) for later phases: `ITEM_ID` (the `id`, driving the branch name and `[karta:item-<id>]` marker); `ITEM_ORACLE` (`type`/`assertions`/`command`, or `opt_out`+`reason`); `ITEM_CONTRACT`, if present; `ITEM_DEPS` (the resolved, all-merged `depends_on` ids); `SERIALIZE`/`SHARED_RESOURCES`, if present (the orchestrator's concern, noted here); `RUN_MODE` — **single-item hatch** vs **orchestrated wave**, an **explicit signal** (`karta-deliver` tells a wave worker so at dispatch; a worker invoked directly with no such signal defaults to single-item) that decides who owns the terminal merge in `build:merge` (see [references/integration-branch.md](references/integration-branch.md)) — never infer it from the integration branch's existence or wave-mates; UI annex fields **only if present** (`COMPONENT_MAP`, `ICON_MAP`, `TOKEN_CHANGES`, `DESIGN_REFERENCE`, `VISUAL_CHECK_WAIVER`, and the binder's `design_facts.source` — a `visual_check_waiver` names the item whose visual gate covers this one via `covered_by`, and routes nothing: `build:acceptance`'s gate choice still keys on this item's own `oracle.type`); and `SME_PACKS` (the binder's `sme` list), resolved and loaded in `build:implement`, self-checked before the gate in `build:acceptance` (6-sme).

### Phase 2 — Sanity-check the item against the codebase  `build:sanity`

**With an `item_context.py` packet, `oracle_cwd`/`oracle_expect`/`dependencies` are already resolved — this phase does not re-derive them.** Read the work item and the binder's `scope`, `design_facts`, and `env_contract`. Then verify the item against the current code:
...
- **Conditional-feature gotchas — UI/visual, data-layer, greenfield tool-pinning, and multi-root oracle mechanics — are in [references/build-annex.md](references/build-annex.md).** Open it the way you'd open `build-ui-data.md` or `build-greenfield.md`: only when the item actually carries that surface.
